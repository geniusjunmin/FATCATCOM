...
| Multiplayer Base | P1 | Prepare the game for connected multi-user play. | Polish friend-request visuals, nav badges, notifications, and invite/search flow beyond prompt-confirm. |
...
- `tools/check-friend-request-contract.js` verifies `/api/friends/requests`, bidirectional accept behavior, client API/sync helpers, friend-panel request UI hooks, and coverage.
...
- `/api/friends/requests` now creates/lists/accepts/rejects friend requests; accepting writes bidirectional relations and snapshots, and the friend panel has send/request/inbox/outbox hooks. Keep it green with `node tools\check-friend-request-contract.js` and `node tools\check-real-friend-online.js`.
