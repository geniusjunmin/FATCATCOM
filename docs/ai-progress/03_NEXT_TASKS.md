...
| --- | --- | --- | --- |
| Server Economy | P0 | Keep authoritative production and resource mutation reliable. | Keep config checks green; shop-state, friend sync, real-friend add, friend rewards, and leaderboard contracts are done. |
...
| Multiplayer Base | P1 | Prepare the game for connected multi-user play. | Replace player-id prompt with a nicer invite/search flow, relation table, and richer social tuning. |
...
- `tools/quick-verify.ps1` runs focused client TS, generated balance, drift, effect coverage, client catalog metadata, shop-state contract, friend-sync contract, real-friend contract, friend-activity contract, friend-reward contract, leaderboard contract, and server tests together.
