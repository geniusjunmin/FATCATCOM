...
| Code Health | P1 | Reduce frontend maintenance risk. | Asset-resolver, formatter, and main-panel config extractions are done; continue splitting HUD, nav, factory, cat, and panel rendering from `BottomNavUI.ts`. |
...
- `tools/quick-verify.ps1` runs focused client TS, generated balance, drift, effect coverage, client catalog metadata, DOM asset resolver contract, DOM formatter contract, main panel config contract, shop-state contract, friend-sync contract, friend visit-scene contract, real-friend contract, friend-activity contract, friend-reward contract, friend-invite contract, friend-request contract, leaderboard contract, and server tests together.
