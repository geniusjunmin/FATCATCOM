...
| Multiplayer Base | P1 | Prepare the game for connected multi-user play. | Polish friend invite/search UX beyond prompt-confirm and refine target-UI notification visuals. |
...
- `tools/check-friend-request-contract.js` verifies `/api/friends/requests`, bidirectional accept behavior, client API/sync helpers, friend-panel request UI hooks, factory friend-entry badge hooks, mail notification surfacing, and coverage.
