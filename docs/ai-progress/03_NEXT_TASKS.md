...
| --- | --- | --- | --- |
| Server Economy | P0 | Keep authoritative production and resource mutation reliable. | Keep config checks green; shop-state, friend sync, real-friend add, friend rewards, friend invite/search, and leaderboard contracts are done. |
...
| Multiplayer Base | P1 | Prepare the game for connected multi-user play. | Add a dedicated relation table, shorter invite codes, and a polished in-panel invite/search UI. |
...
- `tools/check-friend-invite-contract.js` verifies `/api/social/profile`, `/api/friends/search`, invite-code add compatibility, client API/sync helpers, friend-panel search confirmation, and coverage.
...
- `/api/social/profile` and `/api/friends/search` now provide invite-code search before add, and `/api/friends/add` accepts invite codes as well as legacy player ids. Keep it green with `node tools\check-friend-invite-contract.js`.
