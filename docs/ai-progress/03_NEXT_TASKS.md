...
| Multiplayer Base | P1 | Prepare the game for connected multi-user play. | Polish friend-request visuals, notification surfacing, and invite/search flow beyond prompt-confirm. |
...
- `tools/check-friend-request-contract.js` verifies `/api/friends/requests`, bidirectional accept behavior, client API/sync helpers, friend-panel request UI hooks, factory friend-entry badge hooks, and coverage.
