...
| Best Next Move | Continue visual fidelity with generated/Cocos-managed art depth or HUD precision; social next move is friend request notification surfacing and richer target-UI visual polish. |
...
- `SyncManager` supports friend request create/list/accept/reject helpers over `/api/friends/requests`; the DOM friend panel now shows received/sent request summaries, pending counts, send-request action, and accept/reject buttons. The main factory friend entry now fetches pending inbound requests and shows a red badge count.
...
- `node tools\check-friend-request-contract.js`: passed; verifies friend request domain/schema/repository, create/list/accept/reject routes, bidirectional accept behavior, client API/sync helpers, friend-panel request UI hooks, factory friend-entry badge hooks, and service/API coverage.
...
- Friend request inbox/outbox basics and factory friend-entry red badge are implemented. Notification timing, mail-style surfacing, and richer target-UI visual polish still need work.
