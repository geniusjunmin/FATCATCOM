...
| Best Next Move | Continue visual fidelity with generated/Cocos-managed art depth or HUD precision; social next move is friend invite/search UX polish and richer target-UI notification visuals. |
...
- `SyncManager` supports friend request create/list/accept/reject helpers over `/api/friends/requests`; the DOM friend panel now shows received/sent request summaries, pending counts, send-request action, and accept/reject buttons. The main factory friend entry shows a pending-request red badge, and the mail panel surfaces pending friend requests as a notification card that jumps to the friend panel.
...
- Friend request inbox/outbox basics, factory friend-entry red badge, and mail-panel notification surfacing are implemented. Invite/search UX polish and richer target-UI visual treatment still need work.
