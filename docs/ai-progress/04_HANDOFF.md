...
- `renderResearchLines()` intentionally emits 11 horizontal/vertical segments aligned to the six existing node positions. `capture-feature-regression.js` clicks every inventory tab and all three research types at 360x800, 414x896, 430x932, and 768x1024; keep `inventoryTabs`, `inventoryDetailSwitch`, `inventoryUseAction`, `researchEffectSwitches`, and `researchLines` green.
