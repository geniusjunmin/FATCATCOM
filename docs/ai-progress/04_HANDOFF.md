...
- New gate: `node tools\capture-utility-regression.js`. It writes screenshots to `docs/verification/screenshots/2026-06-29-utility-regression/`, clicks the real main-screen hotspots, closes each panel between captures, and asserts panel shell classes plus clean runtime state.
...
- Cat skin tab now has a functional target-like wardrobe layout and four-size screenshot coverage. The next fidelity step should be richer actual skin thumbnails/outfits or story/equipment illustrations, not more geometry churn.
