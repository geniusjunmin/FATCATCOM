...
- Research now has the target `1-2-3-1` seven-node production hierarchy. `RESEARCH_NODE_PRESENTATIONS` maps the three real config ids to target-facing names/levels, while `RESEARCH_PLACEHOLDER_NODES` defines four actionless locked previews. Do not add `data-action` to previews or mutate unlock state locally.
- `renderResearchLines()` emits twelve measured segments. Real nodes expose `data-research-tier`, placeholders expose both `data-research-tier` and `data-research-placeholder`, and the tree declares `data-research-layout="1-2-3-1"`.
- The right detail panel separates current/research-after effects, feedback, prerequisite, point progress, and the existing authoritative action. Display aliases must not replace real ids, costs, descriptions, effects, or manager calls.
- `capture-feature-regression.js` checks seven nodes, `1,2,3,1` tiers, twelve connectors, four actionless placeholders, target display names, no overlap, containment, eight embedded medals, three real switches, and nav clearance at 360x800, 414x896, 430x932, and 768x1024.
- Next safe move is server-backed design for the four previews. Add balance definitions and tests first; only then replace placeholders with live `ResearchManager` entries.
