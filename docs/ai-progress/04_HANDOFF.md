...
- Verification after the latest main factory richness/HUD/side-buttons/bottom-controls passes: `tools/check-client-ts.ps1`, Cocos refresh for `db://assets/scripts`, `node tools/capture-main-regression.js`, and `node tools/verify-ui-clicks-playwright.js` passed.
