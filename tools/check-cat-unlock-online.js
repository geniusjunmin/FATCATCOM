...
const url = `http://localhost:7456/?api=${encodeURIComponent(apiUrl)}&catunlockonline=${Date.now()}`;
...
(async () => {
    ...
    try {
        ...
        const before = await page.evaluate(() => ({
            hasLocked: (document.querySelector("#fatcat-dom-cat-overlay")?.textContent || "").includes("未招募"),
        }));
        await page.click('#fatcat-dom-cat-overlay [data-action="unlockCat"][data-id="c_005"]');
        await page.waitForTimeout(2500);
        const after = await page.evaluate(() => ({
            hasLevel: (document.querySelector("#fatcat-dom-cat-overlay")?.textContent || "").includes("Lv.1/30"),
        ...
        const ok = authRequests.some((item) => item.status === 200)
            && unlockRequests.some((item) => item.status === 200 && item.url.includes("/api/cats/c_005/unlock"))
            && before.hasLocked
            && after.hasLevel
            && after.message.includes("Recruit synced")
            && failedRequests.length === 0
            && messages.length === 0;

        const result = { ok, before, after, authRequests, unlockRequests, failedRequests, messages };
