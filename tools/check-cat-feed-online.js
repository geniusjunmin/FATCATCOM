...
const url = `http://localhost:7456/?api=${encodeURIComponent(apiUrl)}&catfeedonline=${Date.now()}`;
...
(async () => {
    ...
    try {
        ...
        page.on("response", (response) => {
            ...
            if (response.url().includes("/api/cats/") && response.url().includes("/feed")) {
                feedRequests.push({ status: response.status(), url: response.url() });
            }
        ...
        const before = await page.evaluate(() => ({
            weight: document.querySelector("#fatcat-dom-cat-overlay")?.textContent?.match(/\d+\/100/)?.[0] || "",
        }));
        await page.click('#fatcat-dom-cat-overlay [data-action="feedCat"][data-id="c_001"]');
        await page.waitForTimeout(2500);
        ...
        const ok = authRequests.some((item) => item.status === 200)
            && feedRequests.some((item) => item.status === 200 && item.url.includes("/api/cats/c_001/feed"))
            && before.weight === "20/100"
            && after.weight === "21/100"
            && after.message.includes("Feed synced")
            && failedRequests.length === 0
            && messages.length === 0;

        const result = { ok, before, after, authRequests, feedRequests, failedRequests, messages };
